	...
	for (unsigned int i = 0; i < scene ->mNumMaterials; i++)
	{
		...
		if (material->GetTextureCount(aiTextureType_DIFFUSE	))
		{
			...
			if (material->GetTexture(aiTextureType_DIFFUSE,0,&path)==AI_SUCCESS)
			{
				...
				modelBaseName = modelName.substr(modelName.find_last_of("/\\") + 1);
				modelBaseName = modelBaseName.substr(0, modelBaseName.find_last_of("."));

				if (modelBaseName.rfind("hk", 0) == 0)
				{
					texPath = "Textures/Hollow_knight/" + filename;
				}
				...
				else if (modelBaseName.rfind("bs", 0) == 0) {
					texPath = "Textures/Brawl_stars/" + filename;
				}
				else
				{
